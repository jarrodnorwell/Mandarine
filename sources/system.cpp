...
constexpr T System::fast_read(uint8_t *device, uint32_t address)
{
    ...
    assert(std::any_of(checklist.begin(), checklist.end(), [](bool result)
                       { return result; }));
...
constexpr void System::write_peripheral(Peripheral &peripheral, uint32_t address, T value)
{
    ...
    {
        peripheral->write(address, static_cast<uint8_t>(value & 0xFF));
        peripheral->write(address + 1, static_cast<uint8_t>((value >> 8) & 0xFF));
        peripheral->write(address + 2, static_cast<uint8_t>((value >> 16) & 0xFF));
        peripheral->write(address + 3, static_cast<uint8_t>((value >> 24) & 0xFF));
    }
}
