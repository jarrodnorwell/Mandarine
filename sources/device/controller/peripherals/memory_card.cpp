...
namespace peripherals
{
    ...
    uint8_t MemoryCard::handle(uint8_t byte)
    {
        ...
        if (verbose >= 3)
            fmt::print("[MEMCARD_{}] state {}\n", port, state);
        if (command == Command::Read)
            return handleRead(byte);
        if (command == Command::Write)
            return handleWrite(byte);
        if (command == Command::ID)
            return handleId(byte);
        ...
        case 1:
        {
            ...
            {
                if (verbose >= 1)
                    fmt::print("[MEMCARD_{}] Unsupported command 0x{:02x}\n", port, byte);
                state = 0;
            }
    ...
    uint8_t MemoryCard::handleRead(uint8_t byte)
    {
        ...
        case 2:
            state++;
            return 0x5A; // Card ID1
        case 3:
            state++;
            return 0x5D; // Card ID2
        case 4:
        ...
        case 5:
            ...
            {
                if (verbose >= 1)
                    fmt::print("[MEMCARD_{}] Out of bounds read 0x{:04x}\n", port, address._reg);
                address._reg &= 0x3ff;
            }
            else
            {
                if (verbose >= 2)
                    fmt::print("[MEMCARD_{}] Reading 0x{:04x}\n", port, address._reg);
            }
            state++;
            return 0;
    ...
    uint8_t MemoryCard::handleWrite(uint8_t byte)
    {
        ...
        case 5:
            ...
            if (address._reg > 1024 - 1)
            {
                flag.error = 1;
                if (verbose >= 1)
                    fmt::print("[MEMCARD_{}] Out of bounds write 0x{:04x}\n", port, address._reg);
                address._reg &= 0x3ff;
            ...
            {
                if (verbose >= 2)
                    fmt::print("[MEMCARD_{}] Write 0x{:04x}\n", port, address._reg);
            }
            state++;
            return 0;
        ...
        case 137:
            ...
            listener.getBus()->postpone(Event::Controller::MemoryCardContentsChanged{port - 1});
    ...
    uint8_t MemoryCard::handleId(uint8_t byte)
    {
        (void)byte;
        if (verbose >= 1)
            fmt::print("[MEMCARD_{}] Unsupported ID command\n", port);
        command = Command::None;
        return 0xff;
    }
