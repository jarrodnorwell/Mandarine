...
namespace gpu
{
    ...
    {
        listener.listen<Event::Config::Graphics>([&](auto)
                                                 { reload(); });
        reload();
        reset();
    }

    GPU::~GPU() { listener.unlistenAll(); }
    ...
    void GPU::drawTriangle(const primitive::Triangle &triangle)
    {
        ...
        {
            int flags = 0;
            if (triangle.isRawTexture)
                flags |= Vertex::Flags::RawTexture;
            if (gp0_e1.dither24to15)
                flags |= Vertex::Flags::Dithering;
            if (triangle.gouraudShading)
                flags |= Vertex::Flags::GouraudShading;
    ...
    void GPU::cmdPolygon(PolygonArgs arg)
    {
        ...
        for (int i = 0; i < arg.getVertexCount(); i++)
        {
            ...
            if (!arg.gouraudShading || i == 0)
                v[i].color.raw = arguments[0] & 0xffffff;
            if (arg.isTextureMapped)
            {
                if (i == 0)
                    tex.palette = arguments[ptr];
                if (i == 1)
                    tex.texpage = arguments[ptr];
                v[i].uv.x = arguments[ptr] & 0xff;
                v[i].uv.y = (arguments[ptr] >> 8) & 0xff;
                ptr++;
            }
            if (arg.gouraudShading && i < arg.getVertexCount() - 1)
                v[i + 1].color.raw = arguments[ptr++] & 0xffffff;
        }
    ...
    {
        return (x >= drawingArea.left) && (x < drawingArea.right) && (x < VRAM_WIDTH) && (y >= drawingArea.top) && (y < drawingArea.bottom) && (y < VRAM_HEIGHT);
    }
