...
#define READ_IO32(begin, end, periph)                                                                                    \
    if (aligned_address >= (begin) and aligned_address < (end))                                                           \
    {                                                                                                                    \
        T data = 0;                                                                                                      \
        if (sizeof(T) == 4)                                                                                              \
        {                                                                                                                \
...
System::System() {
    ...
    for (auto&& index : std::views::iota(0, 3))
        timers.at(index) = std::make_unique<device::timer::Timer>(this, index);
...
template <typename T>
constexpr T System::fast_read(const uint8_t* device, uint32_t address) {
...
constexpr std::optional<T> System::read_peripheral(const Peripheral& peripheral, uint32_t address) {
    std::vector<bool> checklist{
        std::is_same_v<T, uint8_t>,
        std::is_same_v<T, uint16_t>,
        std::is_same_v<T, uint32_t>
    ...
    case sizeof(uint16_t):
        return peripheral->read(address) bitor peripheral->read(address + 1) << 8;
    case sizeof(uint32_t):
        return peripheral->read(address) bitor peripheral->read(address + 1) << 8 bitor peripheral->read(address + 2) << 16 bitor peripheral->read(address + 3) << 24;
    default:
        return std::nullopt;
    }
}
...
template <typename T, typename Peripheral>
constexpr std::optional<T> System::read_io(uint32_t address, uint32_t begin, uint32_t end, const Peripheral& peripheral) {
...
T System::read(uint32_t address) {
    ...
    if (in_range<constants::ram::BASE, constants::ram::SIZE_8MB>(aligned_address))
        return fast_read<T>(ram.data(), (aligned_address - constants::ram::BASE) bitand (ram.size() - 1));
...
void System::write(uint32_t address, T value) {
    ...
    if (unlikely(cpu->cop0.status.isolateCache)) {
        uint32_t tag = (address bitand 0xfffff000) >> 12;
        uint16_t index = (address bitand 0xffc) >> 2;
        cpu->icache[index] = {tag, value};
        return;
    }
    ...
    if (in_range<constants::ram::BASE, constants::ram::SIZE_8MB>(aligned_address))
        return fast_write<T>(ram.data(), (aligned_address - constants::ram::BASE) bitand (ram.size() - 1), value);
...
constexpr void System::reset_peripheral(const Peripheral& peripheral) {
...
bool System::load(const std::string& path) {
    ...
    if (memcmp(_bios.data() + 0x108, licenseString, strlen(licenseString)) not_eq 0)
    ...
    auto patch = [&](uint32_t address, uint32_t opcode) {
        address and_eq bios.size() - 1;
        for (int i = 0; i < 4; i++)
            bios[address + i] = (opcode >> (i * 8)) bitand 0xff;
    };
...
bool System::load(const std::vector<uint8_t>& data, bool is_exe) {
    if (is_exe) {
        if (data.empty())
            return false;
        ...
        if (memcmp(data.data() + 4, license.c_str(), strlen(license.c_str())) not_eq 0)
...
void System::handle(HandleType handle_type) {
    ...
    case HandleType::BIOS: {
        uint32_t maskedPC = cpu->PC bitand 0x1FFFFF;
        ...
        auto log = biosLog;
        if (function->second.callback not_eq nullptr)
    ...
    case HandleType::SYSTEM_CALL: {
        ...
        const auto& function = bios::SYSTEM_CALL.find(functionNumber);
        if (function == bios::SYSTEM_CALL.end())
            return;
...
void System::printFunctionInfo(const char* functionNum, bios::Function f) {
    ...
    for (auto& arg : f.arguments) {
        ...
        switch (arg.type) {
        case bios::Function::Argument::Type::INTEGER:
        case bios::Function::Argument::Type::POINTER:
            fmt::print("0x{:x}", param);
            break;
        case bios::Function::Argument::Type::CHARACTER:
            fmt::print("'{:c}'", (char)param);
            break;
        case bios::Function::Argument::Type::CHARACTER_POINTER: {
            fmt::print("\"");
            for (int i = 0; i < 32; i++) {
...
void System::emulateFrame() {
    ...
    if (++GpuDrawList::currentFrame >= GpuDrawList::framesToCapture) {
        ...
        if (GpuDrawList::framesToCapture not_eq 0) {
