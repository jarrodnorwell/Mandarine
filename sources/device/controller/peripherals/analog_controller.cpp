...
namespace peripherals
{
    ...
    uint8_t AnalogController::_handle(uint8_t byte)
    {
        ...
        if (command == Command::Read && !analogEnabled)
            return handleRead(byte);
        if (command == Command::Read && analogEnabled)
            return handleReadAnalog(byte);
        if (command == Command::EnterConfiguration)
            return handleEnterConfiguration(byte);
        if (command == Command::ExitConfiguration)
            return handleExitConfiguration(byte);
        if (command == Command::GetLed)
            return handleLedStatus(byte);
        if (command == Command::SetLed)
            return handleSetLed(byte);
        if (command == Command::UnlockRumble)
            return handleUnlockRumble(byte);
        if (command == Command::Unknown_46)
            return handleUnknown46(byte);
        if (command == Command::Unknown_47)
            return handleUnknown47(byte);
        if (command == Command::Unknown_4c)
            return handleUnknown4c(byte);
        ...
        case 1:
        {
            ...
            if (byte == 0x44 && configurationMode)
            { // Set LED status
                command = Command::SetLed;
                state++;
                return ret;
            }
            if (byte == 0x45 && configurationMode)
            { // Get LED status
    ...
    uint8_t AnalogController::handleReadAnalog(uint8_t byte)
    {
        ...
        case 8:
            ...
            {
                listener.getBus()->postpone(Event::Controller::Vibration{port, vibration.small, vibration.big});
                listener.getBus()->process();
            }
    ...
    uint8_t AnalogController::handleUnknown4c(uint8_t byte)
    {
        ...
        case 6:
            state++;
            return (analogEnabled) ? 0x07 : 0x04;
        case 7:
            state++;
            return 0;
        case 8:
